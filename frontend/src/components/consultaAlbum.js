...
export default class ConsultAlbum extends Component {

    constructor(props) {
        super(props);

        this.handleReproductor = this.handleReproductor.bind(this);
        this.handleSharedSong = this.handleSharedSong.bind(this);
    ...
    async componentDidMount() {
        ...
        this.setState({ currentUser: currentUser, useReady: true });

        const token = jwt_decode(currentUser);
        const timesession = token.exp;
        this.id = token.id;

        if (timesession < Date.now() / 1000) {
    ...
    handleReproductorAlb = () => {
        const enlace = '/repAlbum/' + localStorage.getItem("albumVisualizado") ;
        window.location.href = enlace;
    }

    handleReproductor = (id) => {
        const enlace = '/repCancion/' + localStorage.getItem("albumVisualizado") + '/' + id;
        window.location.href = enlace;
    }
